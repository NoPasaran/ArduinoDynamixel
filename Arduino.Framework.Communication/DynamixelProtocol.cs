...
namespace Arduino.Framework.Dynamixel
{
    [Flags]
    ...
    public enum AX12_INSTRUCTION : byte
    {
    ...
    /// <summary>
    /// Implémente l'ensemble des opérations nécessaire pour gérer le bus dynamixel.
    /// </summary>
    public static class DynamixelProtocol
    {
        public const byte AX_BROACAST_ID = 254;
#region method static
        ...
        public static byte[] CreateReadDataInstruction(byte identifiant_dyna, byte startRegister, byte length)
        {
            ...
            result[0] = (byte)AX12_INSTRUCTION.INSTRUCTION_READ_DATA;
            result[1] = identifiant_dyna;
            result[2] = startRegister;
            result[3] = length;
            return result;
        }
        /// <summary>
        ...
        public static byte[] CreatePingInstruction( byte identifiant_dyna)
        {
            ...
            result[0] = (byte)AX12_INSTRUCTION.INSTRUCTION_PING;
            result[1] = identifiant_dyna;
            result[2] = 0;
            return result;
        }
        ...
        public static byte[] WriteInstruction(byte _identifiant_dyna,byte startRegister, byte[] dataToWrite)
        {
            ...
            result[0] = (byte)AX12_INSTRUCTION.INSTRUCTION_WRITE_DATA;
            result[1] = _identifiant_dyna;
            result[2] = startRegister;
            for (byte loop = 0; loop < (byte)dataToWrite.Length; ++loop)
            {
               result[2 + loop] = dataToWrite[loop];
            }
            return result;
        }
        /// <summary>
        /// Permet d'envoyer un message d'écriture au niveau d'un dynamixel donné. Cette écriture n'est
        /// effective que sur la réception d'une instruction ACTION.
        /// </summary>
        ...
        {
            byte lenght = (byte)(3 + dataToWrite.Length );
            byte[] result = new byte[lenght];
            result[0]=(byte)AX12_INSTRUCTION.INSTRUCTION_REG_WRITE;
            result[1] = identifiant_dyna;
            result[2] = startRegister;
            for (byte loop = 0; loop < (byte)dataToWrite.Length; ++loop)
            {
                result[loop+2] = dataToWrite[loop];
            }
            return result;
        }
        ...
        {
            byte[] result = new byte[] {  (byte)AX12_INSTRUCTION.INSTRUCTION_ACTION, (byte)AX_BROACAST_ID};
            return result;
        }
        /// <summary>
        ...
        {
            byte[] result = new byte[] {(byte) AX12_INSTRUCTION.INSTRUCTION_RESET, identifiant_dyna};
            return result;
        }
#endregion
    }
