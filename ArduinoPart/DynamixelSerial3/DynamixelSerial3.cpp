...
//Doit être appelée avant l'envoi de données sur le bus dynamixel
void DynamixelClass::PermutToSend(void)
{
  switchCom(Direction_PinRX,0);
  switchCom(Direction_PinTX,1);
}

//Doit être appelée avant la réception de données sur le bus dynamixel
void DynamixelClass::PermutToReceiv(void)
{
    switchCom(Direction_PinTX,0);
    switchCom(Direction_PinRX,1);
}
// Public Methods //////////////////////////////////////////////////////////////

/* 
 * Permet de récupérer un status packet au niveau du bus dynamixel 
 */
unsigned char DynamixelClass::readStatusPacket(unsigned char idmessage, unsigned char **result)
{
   PermutToReceiv();
   // Récupération de l'entete
   Time_Counter = 0;
   while((availableData() < 4) & (Time_Counter < TIME_OUT))
   {
     Time_Counter++;
     delayus(1000);
   }
   // Vérification et lecture de la trame
     unsigned char identifiant;
     unsigned char iderror;
     unsigned char length; 
     while(availableData()>0)
     {
         Incoming_Byte = readData();
         if((Incoming_Byte == 0xFF) & (peekData() == 0xFF))
         {
           readData();//Lecture du 2nd Start Byte
           identifiant = readData();
           length = readData();
           iderror = readData();
           length = length - 2;
           unsigned char *tmpresult = (unsigned char*)malloc((length+3) * sizeof(unsigned char));
           if(result!=NULL)
           {
               
               Time_Counter = 0;
               while((availableData() < length +1) & (Time_Counter < TIME_OUT))
               {
                 Time_Counter++;
                 delayus(1000);
               }
               tmpresult[0] = idmessage;
               tmpresult[1] = identifiant;
               tmpresult[2] = iderror;
           
               for(unsigned char iloop = 0 ; iloop < length ; ++iloop)
               {
                 tmpresult[iloop+3] = readData();
               }
               length = length+3;
               *result= tmpresult;
               return length;
...
// Permet d'envoyer un "InstructionPacket Generique sur le bus de données"
void DynamixelClass::sendInstructionPacket(unsigned char argc, unsigned char *argv)
{
  unsigned char nbParameters;
  nbParameters = argc - 2;
  unsigned char checksum = 0;
  // > evaluation du checksum byte
  for(unsigned char iloop = 0; iloop < argc; ++iloop)
  {
    checksum += argv[iloop];
  }
  checksum += argc;
  checksum = (~checksum)&0xFF;
  // préparation pour la transmission sur le réseau dynamixel 
  PermutToSend();
  // envoi des données
  sendData(AX_START);
  sendData(AX_START);
  sendData(argv[1]); // identifiant destinataire du message (identifiant dynamixel)
  sendData(argc);    // longueur
  sendData(argv[0]); // code de l'instruction dynamixel
  for(unsigned char iloop = 0 ;iloop < nbParameters; ++iloop)
  {
    sendData(argv[iloop+2]);
  }
  noInterrupts();
  sendData(checksum);
...
void DynamixelClass::begin(long baud, unsigned char directionPinTx, unsigned char directionPinRx)
{	
    Direction_PinTX = directionPinTx;
    Direction_PinRX = directionPinRx;
    setDPin(Direction_PinTX,OUTPUT);
    setDPin(Direction_PinRX,OUTPUT);
    beginCom(baud);
    //> Par défaut on doit être en réception
    PermutToReceiv();
}	
