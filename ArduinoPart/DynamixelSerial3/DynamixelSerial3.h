...
#define TX_DELAY_TIME				10        // Este parametro depende de la velocidad de transmision - pero pueden ser cambiados para mayor velocidad.
#define Tx_MODE                     1
#define Rx_MODE                     0
#define LOCK                        1
...
private:
    ...
    unsigned char Direction_PinTX;      // Broche direction TxD
    unsigned char Direction_PinRX;      // Broche direction RxD
    ...
    void PermutToSend(void);
    void PermutToReceiv(void);
    
    
public:
    
    void begin(long baud, unsigned char directionPinTx,unsigned char directionPinRX);
    void end(void);
    
    // Création d'un InstructionPacket à partir des données en paramètre
    void sendInstructionPacket(unsigned char argc, unsigned char *argv);
    // Récupération d'un StatusPacket 
    unsigned char readStatusPacket(unsigned char idmessage,unsigned char **result);
    
    int reset(unsigned char ID);
    int ping(unsigned char ID); 
    
    // Définit le nouvel identifiant (1-253) pour un actuator donné
    int setID(unsigned char ID, unsigned char newID);
    // Définit la vitesse de communication au niveau du bus série
    int setBD(unsigned char ID, long baud);
    // Définit la nouvelle position de l'AX12
    int move(unsigned char ID, int Position);
    // Définit la nouvelle position de l'AX12 en spécifiant la vitesse
