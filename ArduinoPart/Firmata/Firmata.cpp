...
void FirmataClass::processSysexMessage(void)
{
  ...
  case SYSEXCMD_DYNAMIXEL_VOID_GENERIC:
    ...
    {
      byte bufferLength = (sysexBytesRead - 1) / 2;
      byte *buffer = (byte*)malloc(bufferLength * sizeof(byte));
      byte i = 1;
      byte j = 0;
      while(j < bufferLength){
      ...
      (*currentSysexCallback)(storedInputData[0],bufferLength,buffer);
    }
  case SYSEXCMD_ANALOG_READ :	
  case SYSEXCMD_DYNAMIXEL_INSTRUCTION_PACKET:
    if(currentDynamixelCallback)
    {
      byte bufferLength = (sysexBytesRead - 3) / 2;
      byte *buffer = (byte*)malloc(bufferLength * sizeof(byte));
      byte i = 3;
      byte j = 0;
      byte identmessage = (byte)(storedInputData[1] + (storedInputData[2] << 7)); //identifiant message
      while(j < bufferLength){
        buffer[j] = (byte)storedInputData[i];
        i++;
        buffer[j] += (byte)(storedInputData[i] << 7);
      ...
      (*currentDynamixelCallback)(storedInputData[0],identmessage,bufferLength,buffer);
    }
    break;
  default:
    break;
...
void FirmataClass::debug(const char* string)
{
...
void FirmataClass::attach(byte command, dynamixelCallbackFunction newFunction)
{
  currentDynamixelCallback = newFunction;
}
